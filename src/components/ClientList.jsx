...
import AppointmentForm from "./AppointmentForm"; // ✅ import the form modal
...
export default function ClientList() {
  ...
  const [appointmentToEdit, setAppointmentToEdit] = useState(null); // ✅ for modal
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          ...
          >
            ...
            />
            <button className="w-full px-6 py-3 rounded-full bg-[#8A7563] text-[#F9F4ED] font-semibold hover:bg-[#AD947F] transition-all duration-200 shadow-md text-sm sm:text-base">
              Add Client
            </button>
          </form>
        </div>
                ...
                <li key={c.id} className="py-3">
                  {editingClient?.id === c.id ? (
                    ...
                    <div className="space-y-2">
                      <input
                        className="w-full px-3 py-1.5 rounded-lg border border-[#D2BFAF] bg-white text-[#3B2F2F] focus:outline-none focus:ring-2 focus:ring-[#AD947F] transition-all duration-200"
                      ...
                      />
                      <textarea
                        className="w-full px-3 py-1.5 rounded-lg border border-[#D2BFAF] bg-white text-[#3B2F2F] focus:outline-none focus:ring-2 focus:ring-[#AD947F] transition-all duration-200 min-h-[60px]"
                        placeholder="Notes"
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button
                          ...
                          className="flex-1 sm:flex-none px-4 py-1.5 rounded-full bg-[#8A7563] text-[#F9F4ED] text-sm font-semibold hover:bg-[#AD947F] transition-all duration-200"
                        >
                          Save
                        </button>
                        <button
                          onClick={() => setEditingClient(null)}
                          className="flex-1 sm:flex-none px-4 py-1.5 rounded-full bg-[#D2BFAF] text-[#3B2F2F] text-sm font-semibold hover:bg-[#AD947F] hover:text-white transition-all duration-200"
                        >
                          Cancel
                        </button>
                      </div>
                    ...
                    <div>
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                        <div className="flex-1">
                          <div className="font-semibold text-[#3B2F2F]">
                            {c.name}
                          </div>
                          <div className="text-sm text-[#8A7563]">{c.phone}</div>
                          {c.notes && (
                            <div className="text-sm text-[#8A7563] mt-1">
                              {c.notes}
                            </div>
                          )}
                        </div>
                        <div className="flex gap-2 sm:ml-2">
                          <button
                            ...
                            className="px-3 py-1 rounded-full bg-[#D2BFAF] text-[#3B2F2F] text-xs font-semibold hover:bg-[#AD947F] hover:text-white transition-all duration-200"
                          >
                            Edit
                          </button>
                          <button
                            ...
                            className="px-3 py-1 rounded-full bg-[#FF8A80] text-white text-xs font-semibold hover:bg-red-600 transition-all duration-200"
                          >
                            Delete
                          </button>
                          <button
                            ...
                            className="px-3 py-1 rounded-full bg-[#8A7563] text-white text-xs font-semibold hover:bg-[#AD947F] transition-all duration-200"
                          >
                      ...
                      {isExpanded && (
                        <div className="mt-3 pl-2 border-l-4 border-[#D2BFAF]">
                            ...
                            <ul className="text-sm text-[#8A7563] mb-3">
                              {upcomingAppointments.map((a) => (
                                <li
                                  ...
                                  onClick={() => setAppointmentToEdit(a)} // ✅ open edit modal
                                >
